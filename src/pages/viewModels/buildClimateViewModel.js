...
import { buildComparisonMeta, buildGroupedSeries, buildMovers, buildTableRows, buildTreemapData, buildYearComparisonSeries, buildYearTotals, distinctCount, getSelectedYearValue } from "../../data/selectors";
import { buildDimensionColumns, buildSummaryMeta, buildYearColumns, buildYearComparisonColumn, getComparisonYear, getDimensionLabel, placeKpiFirst } from "./helpers";
...
export function buildClimateViewModel({ dataset, selectedYears, searchQuery, page, currencyDisplay }) {
  ...
  const averageRowValue = summaryRows.length
    ? summaryRows.reduce((sum, row) => sum + (row.valuesByYear?.[mostRecentSelectedYear] || 0), 0) / summaryRows.length
    : 0;
  ...
  const headlineTitle = `${mostRecentSelectedYear} Climate Expenditure`;
  const kpis = [
    {
      title: headlineTitle,
  ...
  return {
    ...
    kpis: placeKpiFirst(kpis, headlineTitle),
