...
export function MoneyTooltip({ active, payload, label, currencyDisplay = "php" }) {
  if (!active || !payload?.length) return null;
  const tooltipLabel = label || payload[0]?.payload?.name || payload[0]?.name || "Value";
  const pointAudit = payload[0]?.payload?.auditByYear?.[payload[0]?.name] || payload[0]?.payload?.audit || null;
    ...
    <div style={{ display: "grid", gap: 8 }}>
      ...
      >
        ...
        {payload.map((entry, index) => {
          ...
          return (
            <div key={index}>
              {(entry.name || "Amount")}: {formatMoney(value, { currencyDisplay })}
            </div>
          );
        })}
      </div>
      {pointAudit ? <AuditTooltipContent audit={pointAudit} /> : null}
    </div>
  );
}

export function DonutTooltip({ active, payload, currencyDisplay = "php" }) {
  if (!active || !payload?.length) return null;
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      ...
      >
        ...
        <div>Amount: {formatMoney(point.value || 0, { currencyDisplay })}</div>
      </div>
...
export function DeltaTooltip({ active, payload, label, currencyDisplay = "php" }) {
  if (!active || !payload?.length) return null;
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      ...
      >
        ...
        <div>Change: {formatSignedMoney(point.delta, { currencyDisplay })}</div>
        <div>{point.startYear}: {formatMoney(point.startValue, { currencyDisplay })}</div>
        <div>{point.endYear}: {formatMoney(point.endValue, { currencyDisplay })}</div>
      </div>
      {point.audit ? <AuditTooltipContent audit={point.audit} /> : null}
    </div>
  );
...
export function DepartmentTreemapNode(props) {
  const { x, y, width, height, index, name, value, currencyDisplay = "php" } = props;
    ...
    <g>
      ...
      {showValue ? (
        <text x={x + 10} y={y + 18 + (labelLines.length * 12) + 4} fill="rgba(255,255,255,.9)" fontSize={11} fontFamily={F.mono}>
          {formatMoneyCompact(value, { currencyDisplay })}
        </text>
      ) : null}
    </g>
  );
