...
import { formatAxisMoneyTick, formatMoneyCompact } from "../../data/formatters";
...
export default function GroupedBarChartCard({
  ...
  const latestSelectedYear = selectedYears[selectedYears.length - 1];
            ...
            <ResponsiveContainer width="100%" height="100%" style={{ userSelect: "none", WebkitUserSelect: "none" }}>
              <PieChart accessibilityLayer={false} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
                ...
                <Tooltip
                  ...
                  content={<DonutTooltip currencyDisplay={currencyDisplay} />}
                />
                <text x="50%" y="52%" textAnchor="middle" fill={C.navy} fontSize={24} fontWeight={700}>
                  {formatMoneyCompact(donutTotal, { currencyDisplay })}
                </text>
              </PieChart>
            </ResponsiveContainer>
              ...
              {donutData.map((item) => (
                ...
                >
                  ...
                  <span style={{ color: C.navy, fontFamily: '"SF Mono", "Menlo", "Consolas", monospace', fontWeight: 700 }}>
                    {formatMoneyCompact(item.value, { currencyDisplay })}
                  </span>
                </div>
              ))}
            </div>
          ...
          <ResponsiveContainer width="100%" height={height} style={{ userSelect: "none", WebkitUserSelect: "none" }}>
            ...
            >
              ...
              <Tooltip
                allowEscapeViewBox={{ x: true, y: true }}
                wrapperStyle={{ zIndex: 20, pointerEvents: "none" }}
                content={<MoneyTooltip currencyDisplay={currencyDisplay} />}
              />
